...
\end{align} in which $\mathbf{\Omega}^*=\frac{1}{2}\nabla\times \mathbf{v}^*$, the macroscopic angular velocity of the fluid, $\tau$ is the relaxation time of the magnetisation of the suspension and $\zeta$ is the kinetic constant. The rest of our code considers non-dimensional equations, so Equation \ref{shliomisDimensional} becomes \begin{align}
...
\end{align} in which $c_1 = \frac{L}{\tau U}$, $c_2 = \frac{L H_0^2}{U\zeta}$ and $H_0$ is the characteristic magnetic field scale. Notice that $\mathbf{M}_0$ is given by \begin{align}
	\mathbf{M}_0 & = M_S L(\alpha|\mathbf{H}|)\hat{\mathrm{e}}_H,
\end{align} in which $M_S$ is a constant, $L(x) = \coth(x) - \frac{1}{x}$ is the Langevin function, $\alpha$ is a paramter and $\hat{\mathrm{e}}_H$ is a unit vector in the direction of the applied field $\mathbf{H}$, namely \begin{align}
...
The applied magnetic field is $\mathbf{H}=(H_x,H_y)$, whose components are given by: \begin{eqnarray}
...
We have performed some tests to assess the influence of the dimensionless parameters $\mathit{Re}$, $\mathrm{C}_\mathrm{pm}$ and $\mathit{Pe}$ on the steady-state regimes that were obtained.  Some Reynolds numbers were chosen and then several tests were executed for some fixed magnetic parameters. The magnetic parameters are $\mathrm{C}_\mathrm{pm}=10.0$, $\alpha = 5.0$ and $\mathit{Pe}\in [0.01,\;1]$. The results here are for Reynolds  50. Figure \ref{Re050nVectorField} is the solution without a magnetic field. Figures \ref{Re050Pe1t02} through \ref{Re050Pe1t20} present streamlines for the velocity when $\mathit{Pe}=1$. Example of a force for this case is presented in Figure \ref{FRe50Pe1}. There is another set of figures for $\mathit{Pe}=1$, namely Figures \ref{Re050Pe001t02} and \ref{Re050Pe001t20}.
...
\caption{Solution for $t=4$ with magnetic field, $\mathit{Re}=50$, $\mathit{Pe}=1$. The vortex moved from bottom to the left wall. \label{Re050Pe1t4}}
\end{figure}
...
Figures \ref{Re050Pe1t02} through  \ref{Re050Pe1t20} show the flow at specific times for $\mathit{Pe}=1$ so that it is possible to see how the vortices evolve over time. At $t=0.2$, a vortex appears at the bottom and moves to the left for some time, as one can observe at at $t=3$. It then moves up, $t=4$. Up to this point, the main vortex didn't change much. As times passes, both vortices grow in size, as one can see at $t=20$, Figure \ref{Re050Pe1t20}. Streamlines of the force are presented in Figure \ref{FRe50Pe1}.
